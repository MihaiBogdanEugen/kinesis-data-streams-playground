...
public final class StockTradesAPISyncProducer {

    private static final Logger LOGGER = LoggerFactory.getLogger(StockTradesAPISyncProducer.class);
    ...
    private static void createStream(final KinesisClient kinesisClient) {
        try {
    ...
    private static void describeStream(final KinesisClient kinesisClient) {
        try {
    ...
    private static void putRecord(final StockTrade stockTrade, final KinesisClient kinesisClient) {
        try {
            ...
            final var putRecordResponse = kinesisClient.putRecord(builder -> builder
                    ...
                    .data(payload)
                    .build());
            LOGGER.info("Record `{}` was put in stream `{}`, with sequence number {} and shard ID {}",
                    stockTrade, STREAM_NAME, putRecordResponse.sequenceNumber(), putRecordResponse.shardId());
        } catch (final JsonProcessingException error) {
            LOGGER.error("Serialization failed for record `{}`", stockTrade, error);
            System.exit(1);
        } catch (final Exception error) {
    ...
    public static void main(final String[] args) throws InterruptedException {
        ...
        LOGGER.info("StockTrades API Sync Producer");

        final var kinesisClient = KinesisClient.builder()
        ...
        for(int i = 0; i < 1000; i++) {
            putRecord(StockTradeGenerator.getRandomStockTrade(), kinesisClient);
            Thread.sleep(5);
        }
    }
}
